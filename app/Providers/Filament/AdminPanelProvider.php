...
class AdminPanelProvider extends PanelProvider
{
    public function panel(Panel $panel): Panel
    {
        return $panel
            ->default()
            ->id('admin')
            // Mounted at the site root (`/`). The previous `/admin` prefix
            // was retired because nothing else on the Laravel domain serves
            // public routes — the marketing site and tenant SPA live on the
            // Nuxt frontend, so this domain is admin-only.
            ->path('/')
            ->login()
            ->passwordReset()
